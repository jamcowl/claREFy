...
import urllib2, ntpath, re, sys, json, scholar, PyPDF2, os, pdfx
#import pdfx
...
def extract_references(txtFileName):
	'''
...
if __name__ == '__main__':
	...
	paper_id = '1412.1633'
#	paper_id = '1504.0821'
	pdfFileName = paper_id+'.pdf'
	txtFileName = paper_id+'.txt'
	pdfurl = 'https://arxiv.org/pdf/'+ pdfFileName
	pdf = pdfx.PDFx(pdfurl)
	metadata = pdf.get_metadata()
	references_list = pdf.get_references()
	
	for reff in references_list:
		print reff.text
	quit()
	
	if not os.path.exists(txtFileName):
		print "converting PDF from internet into txt..."
		os.system("pdfx "+pdfurl+" -t -o "+txtFileName)
		print "done!"
	else:
		print "already had txt file processed!"
	
	# extract the references
	print 'extracting the references from the paper...'
	refs = extract_references(txtFileName)
	print refs
